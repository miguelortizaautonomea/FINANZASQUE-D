...
// POST /api/migrate - Migrar datos del CSV a Supabase
  ...
  try {
    ...
    if (count && count > 0) {
      return NextResponse.json({
        message: `Ya existen ${count} registros. Migración cancelada para evitar duplicados.`,
        existing: count
      });
    }
    ...
    return NextResponse.json({
      success: true,
      message: `✅ Migrados ${inserted} registros a Supabase`,
      total: inserted
    });
  } catch (error: any) {
